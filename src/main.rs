...
fn book_hotel(daftar_booking: &mut Vec<HotelBooking>) {
...
fn show_bookings(daftar_booking: &Vec<HotelBooking>) {
    println!("Data Booking Hotel");
...
fn edit_booking(daftar_booking: &mut Vec<HotelBooking>) {
    ...
    let mut new_booking = book_hotel_input();
    daftar_booking[index] = new_booking;

    println!("Booking berhasil diedit!");
}

fn delete_booking(daftar_booking: &mut Vec<HotelBooking>) {
    show_bookings(&daftar_booking);
...
fn book_hotel_input() -> HotelBooking {
    println!("Masukkan ID Booking:");
    let mut booking_id = String::new();
    io::stdin().read_line(&mut booking_id).expect("Gagal membaca ID Booking");
...
fn main() {
    let mut daftar_booking: Vec<HotelBooking> = Vec::new();

    loop {
        println!("Menu:");
        println!("1. Booking Hotel");
        ...
        match choice.trim().parse() {
            Ok(1) => book_hotel(&mut daftar_booking),
